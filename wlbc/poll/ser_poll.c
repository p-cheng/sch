...
int main()
{
    ...
    int nready;  //接受pool的返回值
    int numbytes;//接受返回值的类型

    char buf[MAXDATASIZE];//发送缓冲区
    struct pollfd client[FILEMAX];//struct pollfd* fds

    //定义IPV4套接口地址结构
    struct sockaddr_in seraddr; //service地址
    struct sockaddr_in cliaddr; //client 地址

    socklen_t  sin_size;

    //初始化IPV4套接字地址结构
    seraddr.sin_family = AF_INET; //指定地址家族
